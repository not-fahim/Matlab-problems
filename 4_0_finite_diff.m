...
figure(xy*10+1)
plot(nodes, y)
title('flux vs postition',[ 'for mesh size = ', num2str(h) , 'cm'] )
xlabel('position (x) cm ')
ylabel('flux(# /cm^2)')

hold on
for i = 1 : length(nodes)
    plot([nodes(i)-h/2 nodes(i)+h/2], [flu(i) flu(i)], 'k-')
    if (i > 1)

    plot([(nodes(i)-h/2) (nodes(i)-h/2)], [flu(i-1) flu(i)], 'k-')
...
hold off
relerror = abs(flu-y)./y;
figure(xy*10+2)
plot(nodes, log(relerror) )
title('log (error) vs log(position(x)) for ' ,['mesh size=' ,num2str(h),'cm'])
xlabel('log(position X (cm))')
